...
class DailyTimeAllocator
{
    ...
    public function allocate(array $intervals, bool $round = true, int $regularLimit = 420): array
    {
        ...
        foreach ($intervals as $interval) {
            ...
            if (! in_array($kind, self::KINDS, true)) {
                $this->invalid('Cần xác nhận loại ca trước khi phân bổ.');
            }
            $start = $this->minute($interval['start']);
            $end = $this->minute($interval['end']);
            if ($end < $start && $kind === 'overtime_evening') {
                $end += 1440;
            }
            if ($round) {
            ...
            if ($start >= 1440) {
                $this->invalid('Giờ vào sau làm tròn thuộc ngày kế tiếp. Chọn dòng đúng ngày làm việc.');
            }
            if ($end <= $start) {
                $this->invalid('Ca không còn thời gian hợp lệ sau làm tròn; cần kiểm tra ảnh.');
            }
        ...
        foreach ($buckets as $kind => $parts) {
            ...
            for ($i = 1; $i < count($parts); $i++) {
                if ($parts[$i - 1][1] !== $parts[$i][0]) {
                    $this->invalid('Nhiều khoảng rời rạc cùng loại ca: cần kiểm tra riêng, không nối qua giờ nghỉ.');
                }
            }
            ...
            if ($minutesKey) {
                $result[$minutesKey] = array_sum(array_map(fn ($part) => $part[1] - $part[0], $parts));
            }
        }
        if ($segments) {
    ...
    public function assertWithinAssignment(array $allocation, ReconciliationRow $row, ?Collection $contextRows = null): void
    {
        ...
        $otherRows = $contextRows === null
            ? $row->period->rows()->where('machine_id', $row->machine_id)->whereKeyNot($row->id)
                ->whereBetween('work_date', [$row->work_date->copy()->subDay()->toDateString(), $row->work_date->copy()->addDay()->toDateString()])->get()
            : $contextRows->filter(fn (ReconciliationRow $other) => $other->id !== $row->id
                && $other->work_date->betweenIncluded($row->work_date->copy()->subDay(), $row->work_date->copy()->addDay()));
            ...
            foreach ($this->ranges($allocation, $row->work_date) as [$start, $end]) {
                foreach ($this->ranges($other->toArray(), $other->work_date) as [$otherStart, $otherEnd]) {
                    if ($start < $otherEnd && $otherStart < $end) {
                        $this->invalid('Giờ máy đã được tính ở dòng BCH hoặc ca qua đêm khác. Kiểm tra các dòng liên quan.');
                    }
                }
            }
        }
        if (($allocation['regular_minutes'] ?? 0) > $this->remainingRegularMinutes($row, $contextRows)) {
            $this->invalid('Tổng giờ hành chính của máy trong ngày vượt 7 tiếng ở các dòng BCH.');
        }
    }

    public function remainingRegularMinutes(ReconciliationRow $row, ?Collection $contextRows = null): int
    {
        $used = $contextRows === null
            ? (int) $row->period->rows()->where('machine_id', $row->machine_id)
                ->whereDate('work_date', $row->work_date)->whereKeyNot($row->id)->sum('regular_minutes')
            : (int) $contextRows->filter(fn (ReconciliationRow $other) => $other->id !== $row->id
                && $other->work_date->isSameDay($row->work_date))->sum('regular_minutes');

        return max(0, 420 - $used);
    }
    ...
    private function ranges(array $values, $date): array
    {
        $ranges = [];
        foreach (self::KINDS as $kind) {
            if (empty($values[$kind.'_start']) || empty($values[$kind.'_end'])) {
                continue;
            }
