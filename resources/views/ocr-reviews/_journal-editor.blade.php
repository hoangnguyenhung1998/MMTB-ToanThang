            ...
            @foreach ($document->rows as $index => $row)
                <article class="journal-row {{ !empty($row->exceptions) ? 'journal-row-alert' : '' }}">
                    ...
                    <div class="journal-row-grid">
                        <label><span>Ngày công</span><input type="date" name="rows[{{ $index }}][work_date]" value="{{ old("rows.$index.work_date", $row->work_date?->format('Y-m-d')) }}"></label>
...
<template id="journalRowTemplate">
    <article class="journal-row">
        ...
        <div class="journal-row-grid">
            <label><span>Ngày công</span><input type="date" data-name="work_date"></label>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    rows?.addEventListener('input', event => { if (event.target.matches('.row-time')) { event.target.value = event.target.value.replace(/[^0-9:]/g, '').slice(0, 5); calculateDuration(event.target.closest('.journal-row')); } });
    ...
    document.getElementById('addJournalRow')?.addEventListener('click', () => {
        ...
        const previousDate = rows.querySelector('.journal-row:last-child input[type="date"]')?.value; if (previousDate) row.querySelector('input[type="date"]').value = previousDate;
