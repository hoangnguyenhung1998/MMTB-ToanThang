                    ...
                    @foreach ($timeGroups as $label => [$startField, $endField])
                        <div class="col-lg col-md-4">
                            <div class="border rounded p-3 h-100 bg-light">
                                ...
                                <input class="form-control form-control-sm mb-2" type="time" name="{{ $startField }}" value="{{ old($startField, $reconciliationRow->{$startField} ? substr((string) $reconciliationRow->{$startField}, 0, 5) : '') }}">
                                <label class="form-label small">Kết thúc</label>
                                <input class="form-control form-control-sm" type="time" name="{{ $endField }}" value="{{ old($endField, $reconciliationRow->{$endField} ? substr((string) $reconciliationRow->{$endField}, 0, 5) : '') }}">
                            </div>
                        </div>
                    @endforeach
                    <div class="col-md-6">
