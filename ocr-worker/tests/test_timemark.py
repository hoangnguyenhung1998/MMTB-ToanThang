...
class TimeMarkTest(unittest.TestCase):
    def test_primary_timemark_complete_early_stops_without_rotation(self):
        engine = Mock()
        with patch("mmtb_ocr_worker.timemark.read_image", return_value=np.zeros((200, 200, 3), dtype=np.uint8)), \
             patch("mmtb_ocr_worker.timemark.rotate", side_effect=lambda image, angle: image) as rotate_image, \
        ...
        self.assertEqual([0], [call.args[1] for call in rotate_image.call_args_list])
    ...
    def test_progress_telemetry_does_not_change_result(self):
        ...
        self.assertEqual("primary_timemark", region)
    ...
    def test_aggregates_non_conflicting_machine_time_and_date_from_separate_crops(self):
        ...
        self.assertEqual("primary_timemark", result.candidate_metadata["machine_evidence"][0]["region"])
        self.assertEqual(3, engine.call_count)
    ...
    def test_same_tier_conflicting_crop_fails_closed(self):
        engine = Mock()
        outputs = iter([
            ([], []),
            (["T-XL0303 11:01 21 Sep,2026"], [0.99]),
            (["T-XL0303 11:31 21 Sep,2026"], [0.99]),
        ])
        with patch("mmtb_ocr_worker.timemark.read_image", return_value=np.zeros((200, 200, 3), dtype=np.uint8)), \
             patch("mmtb_ocr_worker.timemark.flatten_ocr_result", side_effect=lambda *_: next(outputs, ([], []))):
        ...
        self.assertEqual("2026-09-21", result.captured_date)
        self.assertIsNone(result.captured_time)
        self.assertIn("time", result.candidate_metadata["conflicts"])
        self.assertEqual(3, engine.call_count)

    def test_primary_machine_and_date_are_unchanged_when_zero_degree_fallback_supplies_time(self):
        engine = Mock()
        outputs = iter([
            (["VT-LL0008 21 Tháng 9,2026"], [0.99]),
            (["11:01"], [0.99]),
            ([], []),
        ])
        with patch("mmtb_ocr_worker.timemark.read_image", return_value=np.zeros((200, 200, 3), dtype=np.uint8)), \
             patch("mmtb_ocr_worker.timemark.flatten_ocr_result", side_effect=lambda *_: next(outputs, ([], []))):
            result = TimeMarkRecognizer(["VT-LL0008"], engine).recognize(Path("test.jpg"))

        self.assertEqual("VT-LL0008", result.asset_code)
        self.assertEqual("2026-09-21", result.captured_date)
        self.assertEqual("11:01:00", result.captured_time)
        self.assertNotIn("time", result.candidate_metadata["conflicts"])
        self.assertEqual(3, engine.call_count)

    def test_compact_english_timemark_is_complete_in_primary_roi(self):
        engine = Mock()
        with patch("mmtb_ocr_worker.timemark.read_image", return_value=np.zeros((200, 200, 3), dtype=np.uint8)), \
             patch("mmtb_ocr_worker.timemark.flatten_ocr_result", return_value=(["T-XL0303 13:52 21Sep,2026"], [0.99])):
            result = TimeMarkRecognizer(["T-XL0303"], engine).recognize(Path("test.jpg"))

        self.assertEqual("T-XL0303", result.asset_code)
        self.assertEqual("2026-09-21", result.captured_date)
        self.assertEqual("13:52:00", result.captured_time)
        self.assertEqual(1, engine.call_count)
